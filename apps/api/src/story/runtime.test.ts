...
test('start_episode opens a run with his opener as a real message and the authored intents as first choices', async () => {
  ...
  assert.deepEqual(choices.beat, { position: 1, count: 5, kind: 'STORY', hotspots: ['hand'] })
...
test('playing through: the photo beat sends its moment locked, END closes the run, and chat resumes after', async () => {
  const { repo, demo, episode, sent, start, say, run, last, persona } = await setup()
  await start()
